...
fn parse_num(input: &mut &str) -> PResult<Num> {
    ...
    let integer_part = digit1.parse_to::<i64>().parse_next(input)?;
    ...
    let ret: Result<(), winnow::error::ContextError<&str>> = winnow::Parser::value::<(".", ())>().parse_next(input);
    if ret.is_ok() {
        // Parse the fractional part
        let frac_part = digit1.parse_to::<i64>().parse_next(input)?;
    ...
    if exponent != 0 {
        value *= 10f64.powi(exponent.try_into().expect("exponent too large"));
    }
