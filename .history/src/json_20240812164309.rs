...
mod tests {
    ...
    #[test]
fn pest_parse_scientific_notation_should_work() -> Result<()> {
    let input = "1.1e-30";
    let parsed = JsonParser::parse(Rule::number, input)?.next().unwrap();
    let result = parse_value(parsed)?;
    assert_eq!(JsonValue::Number(1.1e-30), result);

    let input = "2.5E+10";
    let parsed = JsonParser::parse(Rule::number, input)?.next().unwrap();
    let result = parse_value(parsed)?;
    assert_eq!(JsonValue::Number(2.5e10), result);

    let input = "-3.14e2";
    let parsed = JsonParser::parse(Rule::number, input)?.next().unwrap();
    let result = parse_value(parsed)?;
    assert_eq!(JsonValue::Number(-3.14e2), result);
