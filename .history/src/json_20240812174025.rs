...
fn parse_num(input: &mut &str) -> PResult<Num> {
    ...
    let mut num = digit1.parse_to::<i64>().parse_next(input)?;
        ...
        .parse_next(input)
    {
        ...
        is_float = true;
        num = num * 10i64.pow(frac.to_string().len() as u32) + frac;
    }
    ...
    if is_float {
        ...
        v *= 10f64.powi(exponent.try_into().expect("exponent out of range"));
