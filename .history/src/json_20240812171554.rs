...
fn parse_num<T>(input: &mut &str) -> PResult<Num> {
    // Process the sign
    ...
    // Initialize the value as a float
    let mut v = num as f64;

    // Check for a decimal point
    if let Ok(_) = ".".value(()).parse_next(input) {
        ...
        .or_else(|_: ErrMode<ContextError>| "E".value(()).parse_next(input))
    {
        ...
        // If parsing the exponent fails, return an error
        return Err(anyhow!(<&str as Into<T>>::into(
            "Invalid scientific notation".into()
        )));
    };
