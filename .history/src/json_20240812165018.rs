...
fn parse_num(input: &mut &str) -> PResult<Num> {
    ...
    let int_part = digit1.parse_to::<i64>().parse_next(input)?;

    // parse the fractional part if it exists
    let frac_part = opt("."
        .value(())
        .parse_next(input)
        .and_then(|_| digit1.parse_to::<i64>().parse_next(input)))
    .unwrap_or(Ok(0));

    // parse the exponent part if it exists
    let exp_part = opt(alt((
        ("e", opt(alt((opt("-"), opt("+"))))),
        ("E", opt(alt((opt("-"), opt("+"))))),
    )))
    .parse_next(input)?;
    ...
    if let Ok(frac) = frac_part {
        if frac != 0 {
            let frac_str = format!("0.{}", frac);
            value += frac_str.parse::<f64>().unwrap();
        }
    }

    // Handle scientific notation
    if let Some((_, exp_sign)) = exp_part {
        let exponent: i64 = digit1.parse_to::<i64>().parse_next(input)?;
        let exp_value = if exp_sign.is_some() {
            exponent
        } else {
            -exponent
        };
        value *= 10_f64.powi(exp_value as i32);
    }
