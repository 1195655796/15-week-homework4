...
fn parse_num(input: &mut &str) -> PResult<Num, ContextError> {
    // Process the sign
    ...
    let num = digit1
        .parse_to::<f64>()
        .parse_next(input)
        .map_err(|e| ContextError::new(e, input))?;

    // Check for scientific notation
    let (num, exponent) = if let Ok(_) =
        ("e".value(())
            .parse_next(input)
            .or_else(|_: ErrMode<ContextError>| {
                <&str as winnow::Parser<&str, &str, ErrMode<ContextError>>>::value::<()>("E", ())
                    .parse_next(input)
                    .map_err(|e| ContextError::new(e, input))
            })?) {
        ...
        (num * 10f64.powi(exponent), exponent)
    } else {
        (num, 0)
    };

    // Determine the final result type (Int or Float)
    let result = if exponent >= 0 && (num as i64) as f64 == num {
        Num::Int(if sign { -(num as i64) } else { num as i64 })
    } else {
        Num::Float(if sign { -num } else { num })
    };
...
fn test_parse_scientific_num() -> PResult<(), ContextError> {
    ...
    let input = "1e4";
    let result = parse_num(&mut (&*input))?;
    assert_eq!(result, Num::Float(10000.0));

    let input = "-1e-4";
    let result = parse_num(&mut (&*input))?;
    assert_eq!(result, Num::Float(-0.0001));
