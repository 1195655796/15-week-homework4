...
fn parse_num(input: &mut &str) -> Result<Num, winnow::error::ContextError<&str>> {
    // Process the sign
    let sign = opt("-").map(|s| s.is_some()).parse_next(input)?;

    // Parse the integer part
    let integer_part: i64 = digit1.parse_to::<i64>().parse_next(input)?;

    // Initialize the fractional part and exponent
    let mut frac = 0.0;
    let mut exponent = 0;

    // Check for a decimal point
    let ret: Result<(), winnow::error::ContextError<&str>> =
        winnow::Parser::value::<()>(".", ()).parse_next(input);
    if ret.is_ok() {
        // Parse the fractional part
        let frac_part: i64 = digit1.parse_to::<i64>().parse_next(input)?;
        let frac_length = frac_part.to_string().len() as f64;
        frac = frac_part as f64 / 10f64.powf(frac_length);
    }

    // Check for exponent part
    if let Ok(_) = winnow::Parser::value::<()>("e", ())
        .parse_next(input)
        .or_else(|_| winnow::Parser::value::<()>("E", ()).parse_next(input))
    {
        // Parse the exponent sign
        let exp_sign = opt("-").map(|s| s.is_some()).parse_next(input)?;
        // Parse the exponent value
        let exp_value: i64 = digit1.parse_to::<i64>().parse_next(input)?;
        // Determine the final exponent value
        exponent = if exp_sign { -exp_value } else { exp_value };
    }

    // Combine integer and fractional parts
    let mut value = integer_part as f64 + frac;
